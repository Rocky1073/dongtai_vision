...
function loadData(dataJson){
    ...
    for(var i=0;i<dataJson.result.length;i++){
        if(dataJson.result[i].coordinatex&&dataJson.result[i].coordinatey){
                var feature = new ol.Feature(new ol.geom.Point([parseFloat(dataJson.result[i].coordinatex),parseFloat(dataJson.result[i].coordinatey)]));
                ...
                for (var i = 0; i < data.highMap.length; i++) {
                    if(data.highMap[i].index==index){
                        mess.result.push({
                            ...
                            "coordinatex": data.highMap[i].lon, "coordinatey": data.highMap[i].lat, "sta": data.highMap[i].item, "index": data.highMap[i].index,
                            "acceptTimeCas":data.highMap[i].acceptTimeCas,"actCas":data.highMap[i].actCas,"nameDep":data.highMap[i].nameDep,"nameHoco":data.highMap[i].nameHoco,"closeTimeCas":data.highMap[i].closeTimeCas
                        })
                    }
            ...
            map.addEventListener('click', function (evt) {
                ...
                if (featureInfo !== undefined && featureInfo !== null
                    ...
                    if(featureInfo.feature.values_.closeTimeCas){
                        html= "<p>立案时间:" + featureInfo.feature.values_.acceptTimeCas + "</p>"+
                            "<p>主办部门:" + featureInfo.feature.values_.nameDep + "</p>"+
                            "<p>承办队员:" + featureInfo.feature.values_.nameHoco + "</p>"+
                            "<p>结案时间:" + featureInfo.feature.values_.closeTimeCas + "</p>"+
                            "<p>违法行为:" + featureInfo.feature.values_.actCas + "</p>";
                    }else{
                        html = "<p>立案时间:" + featureInfo.feature.values_.acceptTimeCas + "</p>"+
                            "<p>主办部门:" + featureInfo.feature.values_.nameDep + "</p>"+
                            "<p>承办队员:" + featureInfo.feature.values_.nameHoco + "</p>"+

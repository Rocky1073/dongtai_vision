...
(function (global, factory) {
  ...
  const FEATURE_SRC =
	  basePath+'/static/hescgis/assets/images/controls/marker.png';
  const FEATURE_SRC_FLAG =
	  basePath+'/static/hescgis/assets/images/controls/flag.png';
  ...
  Html.glass = [
    '<div class="', klasses.glass.control, ' ', klasses.olControl, '">',
      ...
      '></a>',
    '</div>',
    '<ul class="', klasses.glass.result, '" id="', VARS.resultId, '"></ul>'
  ].join('');
  ...
  Html.input = [
    '<div class="', klasses.inputText.control, '">',
      ...
      '></button>',
    '</div>',
    '<ul class="', klasses.inputText.result, '" id="', VARS.resultId, '"  style="display:none"></ul>'
  ].join('');
   ...
   * @class TDT(天地图)董浩然
   *//////////////////////////////////////////////////////////////////////////////////
  var Tianditu = function Tianditu() {

    this.settings = {
      url: '/pubgis/proxy.jsp',
      params: {
    	  url:'http://www.jhmap.gov.cn/api/feature/query',
		  size:999
      }
    };
  };

  Tianditu.prototype.getParameters = function getParameters (options) {
    options.lang = options.lang.toLowerCase();

    return {
      url: basePath+'/gw/buildmap/proxy',
      params: {
    	url:'http://www.jhmap.gov.cn/api/feature/query',
    	tkey:encodeURIComponent(options.query),
		size:options.limit
      }
    };
  };

  Tianditu.prototype.handleResponse = function handleResponse (results) {
	  var count=results.pageinfo.total;
    return results.content.map(function (result) { return ({
      lon: result.geometry.coordinates[0],
      lat: result.geometry.coordinates[1],
      address: {
        name: result.name?result.name:result.address,
        phone: result.phone,
  ...
  Nominatim.prototype.setListeners = function setListeners () {
    ...
    let timeout, lastQuery,thisStart,count;
    ...
    const query = function (evt) {
      ...
      if (hit) {
		this$1.clearResults();
        evt.preventDefault();
        this$1.query(value,0);
      }
    };
	const queryFirst = function (evt) {
		this$1.thisStart=0;
        this$1.query(this$1.lastQuery,this$1.thisStart);
    };
	const queryPre = function (evt) {
		if(this$1.thisStart-this$1.options.limit<0)return;
		this$1.thisStart=this$1.thisStart-this$1.options.limit;
        this$1.query(this$1.lastQuery,this$1.thisStart);
    };
	const queryNext = function (evt) {
	  if(this$1.thisStart+this$1.options.limit>=this$1.count)return;
	  this$1.thisStart=this$1.thisStart+this$1.options.limit;
      this$1.query(this$1.lastQuery,this$1.thisStart);
    };
	const queryEnd = function (evt) {
	  this$1.thisStart=Math.floor((this$1.count-1)/this$1.options.limit)*this$1.options.limit;
      this$1.query(this$1.lastQuery,this$1.thisStart);
    };
	...
	const showResult = function (evt) {
	  if(this$1.container.children[1].children.length>0){
		  this$1.container.children[1].style.display="block";
	  }
    };
	
    this.els.input.addEventListener('keypress', query, false);
    this.els.input.addEventListener('input', handleValue, false);
	this.els.input.addEventListener('click', showResult, false);
  ...
  Nominatim.prototype.query = function query (q,start) {
    var this$1 = this;
    ...
    json(ajax).then(function (res) {
      ...
      switch (this$1.options.provider) {
	    case PROVIDERS.TIANDITU:
		  res_ = res.content.length ?
			this$1.Tianditu.handleResponse(res) : undefined;
		  break;
        case PROVIDERS.OSM:
          res_ = res.length ?
      ...
      const li = createElement(
        'li', '<h5>查询失败！请检查网络或输入！</h5>');
      this$1.els.result.appendChild(li);
	  //this$1.els.result.insertBefore(li,this$1.els.result.childNodes[this$1.els.result.childNodes.length-1]);
    });
  };
  ...
  Nominatim.prototype.createList = function createList (response) {
    ...
    response.forEach(function (row) {
      ...
      const htmlRow = "<span class ='radio' id='radio"+(++i)+"' >";
      const li = createElement('li', htmlRow);
      const html = "<a href=\"#\" title='"+ row.address.name+"'>" + row.address.name + "</a>";
      const div = createElement('li', html);
	  ...
	  const map = this$1.Base.getMap();
      const coord_ = [parseFloat(row.lon), parseFloat(row.lat)];
      const projection = map.getView().getProjection();
      const coord = proj.transform(coord_, 'EPSG:4326', projection);
	  this$1.createFeature(coord,addressHtml);
	  
	  var radioButton=document.getElementById('radio'+i);
	  radioButton.addEventListener('click', function (evt) {
        //evt.preventDefault();
		
		this$1.radioClick(row,radioButton);
		
      }, false);
    });
	this$1.Base.getMap().getView().fit(this.getSource().getExtent(), { duration: 500 });
    ...
    if (this.options.preventDefault === true) {
      ...
      if (bbox) {
        map.getView().fit(bbox, { duration: 500 });
    	  ...
    	  if(this.options.provider==="tianditu")
    		{
    	    	flyTo(map, coord,500,0.0000107288360595703125);
    		}
    	    else{flyTo(map, coord);}
      }
      const feature = this.createFeature(coord, addressHtml);
  ...
  Nominatim.prototype.radioClick = function radioClick (row,radioButton) {
	var radios=document.getElementsByClassName('radio');
	for(var k=0;k<radios.length;k++)
	{
		radios[k].style.background="url(../zhcg_mqsb/static/hescgis/assets/images/controls/radioUnCheck.png) no-repeat";
		radios[k].style.backgroundPosition= "10px";
	}
	radioButton.style.background="url(../zhcg_mqsb/static/hescgis/assets/images/controls/radioChecked.png) no-repeat"; 
	radioButton.style.backgroundPosition= "10px";
    ...
    this.addLayer();
    feature.setStyle(this.options.featureStyleFlag);
  ...
  //点击结果列表后创建feature即marker
  Nominatim.prototype.createFeature = function createFeature (coord,address) {
	  //创建marker前先清空
	  //this.getSource().clear();
    const feature = new Feature(new Point(coord));
    this.addLayer();
  ...
  Nominatim.prototype.addressTemplate = function addressTemplate (address) {
    let html = [];
    if (address.name) {
      html.push(['<span class="', klasses$1.road, '">名称：{name}</span>'].join(''));
    }
	...
	if (address.address|| address.phone) {
      html.push([
        '<span class="', klasses$1.address, '">地址：{address}</br> 电话：{phone}</span>'
      ].join(''));
    }
  ...
  Nominatim.prototype.getProvider = function getProvider (options) {
    ...
    switch (options.provider) {
      case PROVIDERS.TIANDITU:
        provider = this.Tianditu.getParameters(options);
        break;
      case PROVIDERS.OSM:
  ...
  Nominatim.prototype.listenMapClick = function listenMapClick () {
    ...
    mapElement.addEventListener('click', {
      handleEvent: function (evt) {
		...
		this_.els.container.children[1].style.display="none";
  ...
  Nominatim.prototype.clearResults = function clearResults (collapse) {
    collapse && this.options.targetType === TARGET_TYPE.GLASS
	  ...
	  this.els.container.children[1].style.display="none";
	  this.getSource().clear();
  ...
  var Base = (function (Control$$1) {
    function Base(type, options) {
      ...
      DEFAULT_OPTIONS.featureStyle = [
        new Style({ image: new Icon({ scale: .7, src: FEATURE_SRC,anchor: [0.5, 1] }) })
      ];
	  DEFAULT_OPTIONS.featureStyleFlag = [
        new Style({ image: new Icon({ src: FEATURE_SRC_FLAG,anchor: [0.2308, 0.9333] }) })
      ];
